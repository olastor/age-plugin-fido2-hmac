...
> [!NOTE]
> This plugin by default uses FIDO2's hmac-secret extension for generating an X25519 age identity, which allows for encryption without the token, but also means a compromise of the identity would affect _all_ files encrypted to the same recipient (as with a normal age identity).
>
