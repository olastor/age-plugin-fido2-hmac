package plugin

import (
	"fmt"
	"github.com/olastor/go-libfido2"
	"reflect"
	"testing"
)

func TestEnforcePin(t *testing.T) {
	locs, err := libfido2.DeviceLocations()
	if err != nil {
		t.Error(err)
	}

	var dev *libfido2.Device
	if len(locs) != 1 || locs[0].Product != "Nitrokey 3" {
		fmt.Printf("Testing with virtual test device not possible\n")
		return
	}

	dev, _ = libfido2.NewDevice(locs[0].Path)
	info, _ := dev.Info()
	if info == nil {
		fmt.Printf("Testing with virtual test device not possible.\n")
		return
	}
	aaguid := string(info.AAGUID)
	if aaguid != "AAGUID0123456789" {
		fmt.Printf("Testing with virtual test device not possible.\n")
		return
	}

	// Wrap the raw device in the interface wrapper
	device := NewLibFido2Device(dev)

	// these were generated with the simulated device and the ifs=e2e/test_device.bin
	// recStr := "age14wj87kklx0nm6ek9sze0n4un2xrctweh37gw2hqxyl0h5asf633qtpymdv"
	idStr := "AGE-PLUGIN-FIDO2-HMAC-"

	i, err := ParseFido2HmacIdentity(idStr)
	if err != nil {
		t.Error(err)
	}

	secretPin, err := device.GetHmacSecret(i.CredId, i.Salt, "1234")
	if err != nil {
		t.Error(err)
	}

	secretNoPin, err := device.GetHmacSecret(i.CredId, i.Salt, "")
	if err != nil {
		t.Error(err)
	}

	if reflect.DeepEqual(secretNoPin, secretPin) {
		t.Error("Secrets with or without PIN must not be the same")
	}
}
